...
if not use_preloaded_data:
    ...
    data = data.loc[data["state"]=="bavaria"]
    ...
    # add no snow when none was reported
    ...
    X = np.zeros((data.shape[0], lag, len(data.columns)-4))
    # X[:, :, :-1] = np.nan
    length = np.zeros(data.shape[0])

    scaler = StandardScaler()
    X[:, 0, 3:-3] = scaler.fit_transform(data.iloc[:, range(7, len(data.columns)-3)].values)
    X[:, 0, -3:-1] = data.iloc[:, -3:-1].values
    X[:, 0, 0:3] = data.iloc[:, 4:7].values
    xcols = data.iloc[:, range(4, len(data.columns)-1)].columns
    ...
    j = 0
    max_j = len(data.groupby(["country", "state", "parameter"]))
    for name, group in data.groupby(["country", "state", "parameter"]):
    ...
    np.save("data/sequenzializedData_bavaria2.npy", X)
    np.save("data/sequenzializedLength_bavaria2.npy", length)
    np.save("data/sequenzializedY_bavaria2.npy", y)
    np.save("data/XColNames2.npy", np.array(xcols))

X = np.load("data/sequenzializedData_bavaria2.npy")
length = np.load("data/sequenzializedLength_bavaria2.npy")
y = np.load("data/sequenzializedY_bavaria2.npy")
...
#network
class Simple_Weather_Net(nn.Module):
    def __init__(self, embedding_dim):
        super(Simple_Weather_Net, self).__init__()

        self.lstm = nn.LSTM(embedding_dim, 512, num_layers=3, bidirectional=False, batch_first=True)
...
#initialize model
model = Weather_Net2(embedding_dim=12)
...
def make_predictions(data_loader):
    ...
    return test_preds.detach().numpy()
