...
for parameter, supgroup in data.groupby('parameter'):
    for state, group in supgroup.groupby('state'):
        ...
        if len >= 100:
            ...
            param_grid = {'max_depth': [16, 64, 256], 'n_estimators': [128, 256, 512]}

            rf_regr = RandomForestRegressor()
            search = GridSearchCV(rf_regr, param_grid, cv=2, scoring='r2').fit(group.iloc[:, range(5, len(data.columns)-1)].values, group.iloc[:, len(data.columns)-1].values)

            print("{} {}: {}".format(state, parameter, search.best_score_))
            results.append([state, parameter, search.best_score_])

            np.save("results" + parameter + state + ".npy", search.cv_results_)
            dump(search.best_estimator_, state + ".joblib")
