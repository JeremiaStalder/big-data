...
for parameter, group in data.groupby('parameter'):
    ...
    train_X, test_X, train_y, test_y = train_test_split(group.iloc[:, range(5, len(data.columns)-1)].values, group.iloc[:, len(data.columns)-1].values, test_size=0.30)
    ...
    np.save("results/parameter/results" + parameter + ".npy", results)
