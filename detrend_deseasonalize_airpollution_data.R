...
############## INSERT TEMPERATURE DATA IF AVAILABLE
  ...
  # average if multiple statios with 
...
### USE MA -DATA for airpollution
  
  # replace region with country in openaq if no match in mobility data
  indicator_change_region_name = sort(match(unique(global_mobility_report_clean_stringency_index$sub_region_1), unique(openaq_state_ma_monthly$sub_region_1))) # identify matched regions
  ...
  openaq_state_ma_monthly[(openaq_state_ma_monthly$sub_region_1 %in% not_matched_regions),]$sub_region_1 = openaq_state_ma_monthly[(openaq_state_ma_monthly$sub_region_1 %in% not_matched_regions),]$CountryCode
  
  # test match
  indicator_change_region_name_1 = sort(match(unique(global_mobility_report_clean_stringency_index$sub_region_1), unique(openaq_state_ma_monthly$sub_region_1))) 
  not_matched_regions_1 = unique(openaq_state_ma_monthly$sub_region_1)[-indicator_change_region_name_1]
  ...
  merged_data = inner_join(global_mobility_report_clean_stringency_index, openaq_state_ma_monthly, by = c("CountryCode", "sub_region_1", "Date")) 
  summary(merged_data)
...
# Airpollution Data Step 2 ---- 
    ...
    list_country_code_region = select(global_mobility_report_clean_stringency_index, CountryCode, Region) %>%
      distinct()
    openaq_state_ma_monthly = left_join(openaq_state_ma_monthly, list_country_code_region, by=("CountryCode"))
    
    # Average over new subregions after merging
    openaq_state_ma_monthly = group_by(openaq_state_ma_monthly, CountryCode, Date, sub_region_1, parameter, unit) %>%
...
# Descriptive Airpollution ----   
  ...
  # worldwide
  data_wide = group_by(openaq_state_ma_monthly, parameter, Date) %>%
  ...
  data_plot = filter(openaq_state_ma_monthly, CountryCode %in% list_countries) %>%
  ...
  # difference to previous year
    openaq_state_ma_monthly_previous_year = mutate(openaq_state_ma_monthly, Date = Date+years(1))  %>%
